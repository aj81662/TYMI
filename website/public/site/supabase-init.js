(async function(){
  ...
  for (const envPath of envCandidates) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        SUPABASE_KEY = j.supabaseKey || j.SUPABASE_KEY || j.SUPABASE_ANON_KEY || '';
  ...
  if (!SUPABASE_KEY) SUPABASE_KEY = getMeta('supabase-anon-key');

  // Last-resort fallback for bundled/offline contexts (Expo asset iframe/WebView)
  // where env.json/meta tags are not reachable.
  if (!SUPABASE_URL) SUPABASE_URL = 'https://sxucfwshtvskoqvisbeu.supabase.co';
  if (!SUPABASE_KEY) SUPABASE_KEY = 'sb_publishable_asZW_2Wkn-ofTq_LsqUxhw_GvSv0J_K';

  if (!SUPABASE_URL || !SUPABASE_KEY) {
    console.warn('Supabase configuration not found. Add /env.json or <meta name="supabase-url"> and <meta name="supabase-key"> to each login/dashboard page.');
    return;
  }
