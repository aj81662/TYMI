...
Use environment variables for project credentials instead of embedding them in the repo.
...
Store real secrets in a local env file (for Vite: `.env.local`) and never commit them. The `service_role` key must never be exposed to client code.
...
npm install -D supabase  # Supabase CLI / type generation
```
...
// Read credentials from environment (Vite uses `VITE_` prefix). See .env.example
