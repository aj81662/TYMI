  ...
  ns.signupAndCreateUser = async function(username, password, role, preferredDoctor, doctorSpecialties) {
    ...
    try {
      ...
      ({ data: authData, error: authError } = await supabase.auth.signUp({
        email,
        password,
        options: { data: { username, role, doctor_specialties: doctorSpecialties || '' } }
      }));
      ...
      try {
        ...
        if (role === 'doctor') {
          let doctorErr = null;
          ({ error: doctorErr } = await supabase.from('doctors').upsert({
            user_key: userId,
            display_name: username,
            email,
            specialties_text: doctorSpecialties || null,
          }, { onConflict: 'user_key' }));
          if (doctorErr) {
            ({ error: doctorErr } = await supabase.from('doctors').upsert({
              user_key: userId,
          ...
          if (doctorSpecialties) {
            try {
              await supabase
                .from('users')
                .update({ doctor_specialties_text: doctorSpecialties, updated_at: new Date().toISOString() })
                .eq('user_key', userId);
            } catch (e) {
              console.warn('users doctor specialties update skipped:', e?.message || e);
            }
          }
        }
